...
<g:if test="${models}">
    <ul style="list-style-type:none" class="list-group">
        <g:each status="j" in="${models}" var="model">
            ...
            <g:if test="${model.contains}">
                <table class="table table-striped">
                    ...
                    <mc:relationships element="${model}" var="relationship" direction="outgoing" type="containment">
                        ...
                        %>
                        <tr >
                            <td width="100">${relationship.destination.name}</td>
                            <td width="200">${relationship.destination.description}</td>
                            <td width="50">GE${relationship.destination.id}</td>
                            <td width="50"><a href="#${relationship.destination.valueDomain?.id}">${relationship.destination.valueDomain?.name}</a></td>
                            <td width="100">
            ...
            <g:if test="${model?.parentOf}"> <g:render template="linkedModel2" model="${[models: model.parentOf, index: j+1]}"/></g:if>
            </li>
        </g:each>
    </ul>
</g:if>